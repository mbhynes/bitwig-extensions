...
function InitNoteInputs(deviceName)
{
	noteInputs[0] = host.getMidiInPort(0).createNoteInput(deviceName + "(CH1)", "?0????" );
    noteInputs[1] = host.getMidiInPort(0).createNoteInput(deviceName + "(CH2)", "?1????");
    noteInputs[2] = host.getMidiInPort(0).createNoteInput(deviceName + "(CH3)", "?2????");
    noteInputs[3] = host.getMidiInPort(0).createNoteInput(deviceName + "(CH4)", "?3????");
    noteInputs[4] = host.getMidiInPort(0).createNoteInput(deviceName + "(CH5)", "?4????");
    noteInputs[5] = host.getMidiInPort(0).createNoteInput(deviceName + "(CH6)", "?5????");
    noteInputs[6] = host.getMidiInPort(0).createNoteInput(deviceName + "(CH7)", "?6????");
    noteInputs[7] = host.getMidiInPort(0).createNoteInput(deviceName + "(CH8)", "?7????");
    noteInputs[8] = host.getMidiInPort(0).createNoteInput(deviceName + "(CH9)", "?8????");
    noteInputs[9] = PadNotes = host.getMidiInPort(0).createNoteInput(deviceName + "Pads", "DF????","EF??", "AF????");
    noteInputs[10] = host.getMidiInPort(0).createNoteInput(deviceName + "(CH11)", "?A????");
    noteInputs[11] = host.getMidiInPort(0).createNoteInput(deviceName + "(CH12)", "?B????");
    noteInputs[12] = host.getMidiInPort(0).createNoteInput(deviceName + "(CH13)", "?C????");
    noteInputs[13] = host.getMidiInPort(0).createNoteInput(deviceName + "(CH14)", "?D????");
    noteInputs[14] = host.getMidiInPort(0).createNoteInput(deviceName + "(CH15)", "?E????");
    noteInputs[15] = host.getMidiInPort(0).createNoteInput(deviceName + "(CH16)", "?F????");
	...
	for (var x = 0; x < 16; x++)
	{
		noteInputs[x].setShouldConsumeEvents(false);
	}
}
...
function ControlsMidi(status,data1, data2) {
    ...
    if (status == bankAStatus) {
        ...
        if ( PRODUCT_ID == MPK225_PID) {
            if (data1 >= K1 && data1 < K1 + 4) {
                trackBank.getTrack(data1 - K1).getVolume().inc(uint7ToInt7(data2), 128);
            }
            if (data1 >= K1 + 4 && data1 < K1 + 8) {
                trackBank.getTrack(data1 - K1 - 4).getPan().inc(uint7ToInt7(data2), 128);
            }

            else if (data1 >= S_225_BankB && data1 <= (S_225_BankB + 4)) {
                if (pressed == true) {
                    trackBank.getTrack(data1 - S_225_BankB).getArm().toggle();
                }
                else {
                    updateButtonLeds();
                }
            }
            else if (data1 >= S_225_BankC && data1 <= (S_225_BankC + 4)) {
                if (pressed == true) {
                    trackBank.getTrack(data1 - S_225_BankC).select();
                }
                else {
                    updateButtonLeds();
                }
        ...
        if ( PRODUCT_ID == MPK225_PID) {
            ...
            if (data1 >= K1 + 4 && data1 < K1 + 8) {
                trackBank.getTrack(data1 - K1 - 4).getSend(1).inc(uint7ToInt7(data2), 128);
            }
        }
        else if (data1 >= K1 && data1 < 60) {
